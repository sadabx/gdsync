...
pub(crate) enum Commands {
    ...
    Sync {
        ...
        /// Bypass bulk deletion safety checks
        #[arg(long)]
        force: bool,
    },
    ...
    Watch {
        ...
        /// Bypass bulk deletion safety checks during initial sync
        #[arg(long)]
        force: bool,
    },
    ...
    match cli.command {
        ...
        Some(cmd) => match cmd {
            ...
            Commands::Sync {
                ...
                permanent_delete,
                force,
            } => handle_sync(path, dry_run, concurrency, permanent_delete, force).await?,
            Commands::Watch {
                path,
                debounce_ms,
                concurrency,
                ...
                notify,
                force,
            } => handle_watch(path, debounce_ms, concurrency, permanent_delete, notify, force).await?,
...
pub(crate) async fn handle_auth(
    ...
    let (cid, csec) = if let Some(id) = client_id {
        ...
        (
            ...
            gdsync_core::auth::DEFAULT_CLIENT_SECRET.map(ToString::to_string),
        )
    };
